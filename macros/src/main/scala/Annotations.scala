...
class FirebaseData() extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any = macro FirebaseDataImpl.impl
}

object FirebaseDataImpl {
...
//        q"""
//            ${TermName(field)} = firebase4s.macros.FirebaseDataImpl.obfuscateValue(this.${TermName(field)})
//          """
...
//      val newToString = extractNewToString(sensitiveFields)
      ...
      val FireBaseDataT = q"""FirebaseDataTree"""

      val newParents = parents.size match {
          case 0 => List(FireBaseDataT)
          case _ => parents :+ FireBaseDataT
        }

      val hello = q"""def hello(): Unit = println("hello from macro")"""

      c.Expr[Any](
        q"""
        case class $className ( ..$params ) extends ..$newParents {
          ..$body
          ..$hello
        }
      """
